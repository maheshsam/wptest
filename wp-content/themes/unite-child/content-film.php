...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header page-header">
		...
		<div class="entry-meta">
			<div class="row">
				<?php $terms = wp_get_post_terms( $post->ID, array( 'film-genre' ) ); ?>
				<div class="col-md-4">
					<?php foreach ( $terms as $term ) : ?>
					<label class="label label-info"><?php echo $term->name; ?></label>
					<?php endforeach; ?>
					<?php $terms = wp_get_post_terms( $post->ID, array( 'film-country' ) ); ?>
					<?php foreach ( $terms as $term ) : ?>
					<label class="label label-warning"><?php echo $term->name; ?></label>
					<?php endforeach; ?>
				</div>
				<div class="col-md-4">
					<?php 
					$release_date_value = get_post_meta( $post->ID , 'release_date', true ); 
					if ( ! empty( $release_date_value ) ) { 
					    echo "Release date: ". substr($release_date_value, 6 , 2) ."-". substr($release_date_value, 4 , 2) ."-".substr($release_date_value, 0 , 4);
					}
				?>
				</div>
				<div class="col-md-4">
					<?php
					$price_value = get_post_meta( $post->ID , 'ticket_price', true ); 
					if ( ! empty( $price_value ) ) { 
					    echo "Ticket Price: $". $price_value;
					}
					?>
				</div>
			</div>
